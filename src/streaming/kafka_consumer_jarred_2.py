...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_jarred_2.csv"
...
def process_message(row: dict[str, Any]) -> dict[str, Any]:
    ...
    Small modification: Add a 'processed_by' tag to the message.
    """
    LOG.info("Processing raw message and adding enrichment tag.")

    # ADD THIS LINE:
    row["processed_by"] = "Jarred-Consumer"
...
def consume_messages(consumer: Any) -> int:
    ...
    while consumed_count < MAX_MESSAGES:
        ...
        # Track running stats (e.g., total revenue)
        price = float(processed.get("price", 0.0))
        total_revenue += price
        LOG.info(f"Cumulative Revenue: ${total_revenue:.2f}")
